...
### 6.2 充值VIP制积分体系验收
5. **积分系统**：注册用户获得6积分，生成成功扣除2积分，失败不扣分。
6. **签到机制**：Free用户每周签到获得6积分，Plus/Pro用户每日签到获得6积分。
7. **用户权限控制**：Free用户1张衣物，Plus用户3张衣物，Pro用户10张衣物批量生成。
8. **用户等级升级**：加微信升级Plus，购买积分包升级Pro，等级权限正确分配。
9. **积分永久性**：积分永不清零，累积使用机制正常工作。
10. **线下交易记录**：积分充值记录完整保存，等级升级自动触发。

### 6.3 技术架构验收  
11. **数据库集成**：Turso SQLite连接正常，用户数据持久化存储。
12. **签到记录表**：user_checkins表正常工作，防重复签到机制有效。
13. **AI接口调用**：APIcore AI接口调用成功率 >95%，重试机制正常工作。
14. **认证系统**：NextAuth.js邮箱密码登录注册功能正常。
15. **部署运行**：Cloudflare Pages部署成功，访问稳定。
16. **积分充值系统**：线下交易记录和自动等级升级功能正常。
