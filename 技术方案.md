...
### 1.1 项目概述
小猫更衣是一个基于AI的虚拟试穿平台，允许用户上传个人照片和服装图片，通过AI生成真实感强的虚拟试穿效果。项目采用积分会员制，提供Free和Plus两种用户等级，实现商业化运营。

### 1.2 核心功能
- **AI虚拟换衣**：集成APIcore AI引擎，实现高质量图像生成
- **积分会员体系**：1.5积分/次消费，每日10积分赠送，Plus会员批量生成特权
- **用户管理**：NextAuth.js认证，邮箱+密码注册登录
- **兑换码系统**：微信兑换码订阅，无需对接支付系统
...
CREATE TABLE users (
  ...
  subscription_type TEXT DEFAULT 'free' CHECK (subscription_type IN ('free', 'plus_monthly', 'plus_yearly')),
  credits INTEGER DEFAULT 10, -- 积分余额，新用户注册赠送10积分
...
#### 3.2.2 兑换码表 (redemption_codes)
```sql
CREATE TABLE redemption_codes (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  code TEXT UNIQUE NOT NULL, -- 兑换码，建议8-12位随机字符
  credits INTEGER NOT NULL, -- 兑换获得的积分数
  subscription_type TEXT, -- 兑换的订阅类型
  subscription_days INTEGER, -- 订阅天数（月度30天，年度365天）
  used_by_user_id INTEGER, -- 使用兑换码的用户ID
  used_at DATETIME, -- 使用时间
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  expires_at DATETIME, -- 兑换码过期时间
  FOREIGN KEY (used_by_user_id) REFERENCES users (id) ON DELETE SET NULL
);

-- 索引优化
CREATE INDEX idx_redemption_code ON redemption_codes(code);
CREATE INDEX idx_redemption_used ON redemption_codes(used_by_user_id, used_at);
```

#### 3.2.3 生成历史表 (generation_history)
```sql
CREATE TABLE generation_history (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL,
  credits_used DECIMAL(3,1) NOT NULL, -- 使用的积分数，支持1.5积分
...
#### 3.2.4 每日积分记录表 (daily_credits_log)
```sql
CREATE TABLE daily_credits_log (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL,
  credits_awarded INTEGER DEFAULT 10, -- 发放的积分数
  award_date DATE NOT NULL, -- 发放日期，格式：YYYY-MM-DD
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(user_id, award_date), -- 防止同一用户同一天重复发放
  FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE
);

-- 索引优化
CREATE INDEX idx_daily_credits_date ON daily_credits_log(award_date DESC);
CREATE INDEX idx_daily_credits_user ON daily_credits_log(user_id, award_date DESC);
```
...
export class AIService {
  private static readonly API_URL = 'https://api.apicore.ai/v1/chat/completions';
